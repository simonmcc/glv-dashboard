...
function StatusDot({
  ...
  return (
    <span
      className={`inline-block w-2.5 h-2.5 rounded-full ${getDotColor(status, colorMap)}`}
    />
  );
}
...
function EmptySection({ label }: { label: string }) {
  return <div className="text-sm text-gray-400 py-2">No {label} found</div>;
}
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-4">
        ...
        <section
          className="bg-white rounded-lg shadow-sm border"
          data-testid="joining-journey-section"
        >
          <h2 className="text-base font-semibold text-gray-900 px-4 py-3 border-b bg-gray-50 rounded-t-lg">
            Joining Journey
          </h2>
          ...
          joiningJourneyState === "loading" ? (
            <SectionPlaceholder
              state={joiningJourneyState}
              label="joining journey data"
            />
          ) : (
            <div className="divide-y divide-gray-100">
              {memberJoiningJourney.length === 0 ? (
                <div className="px-4">
                  <EmptySection label="joining journey records" />
                </div>
              ) : (
                memberJoiningJourney.map((r, i) => {
                  ...
                  return (
                    <div key={`${r.Item}-${i}`}>
                      <div
                        className={`flex items-center justify-between px-4 py-3 ${joiningStatusColors[r.Status] || "bg-white border-gray-200"}`}
                      >
                        <div className="flex items-center gap-3">
                          <StatusDot
                            status={r.Status}
                            colorMap={joiningStatusColors}
                          />
                          <span className="font-medium text-sm">{r.Item}</span>
                          {(r.Item === "Criminal Record Check" ||
                            r.Item === "Internal Check") && (
                      ...
                      {isGrowingRoots &&
                        GR_MODULES.map((grModule) => {
                          ...
                          return (
                            <div
                              key={grModule.name}
                              className={`flex items-center justify-between pl-10 pr-4 py-2 border-t border-gray-100 ${colorClass}`}
                            >
          ...
          {disclosuresState === "idle" || disclosuresState === "loading" ? (
            ...
            <div className="divide-y divide-gray-100">
              {memberDisclosures.length === 0 ? (
                <div className="px-4">
                  <EmptySection label="disclosure records" />
                </div>
              ) : (
                memberDisclosures.map((r, i) => (
                  <div
                    key={`${r["Disclosure authority"]}-${i}`}
                    className={`flex items-center justify-between px-4 py-3 ${disclosureStatusColors[r["Disclosure status"]] || "bg-white border-gray-200"}`}
                  >
                    <div className="flex items-center gap-3">
                      <StatusDot
                        status={r["Disclosure status"]}
                        colorMap={disclosureStatusColors}
                      />
                      <span className="font-medium text-sm">
                        {r["Disclosure authority"]}
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-white/60">
                      ...
                      {r["Disclosure expiry date"] && (
                        <span className="text-xs text-gray-600 tabular-nums">
                          {formatDate(r["Disclosure expiry date"])}
                        </span>
                      )}
                    </div>
                  </div>
        ...
        <section
          className="bg-white rounded-lg shadow-sm border"
          data-testid="team-reviews-section"
        >
          <h2 className="text-base font-semibold text-gray-900 px-4 py-3 border-b bg-gray-50 rounded-t-lg">
            Team Directory Reviews
          </h2>
          {teamReviewsState === "idle" || teamReviewsState === "loading" ? (
            <SectionPlaceholder
              state={teamReviewsState}
              label="team review data"
            />
          ) : (
            <div className="divide-y divide-gray-100">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {r["Scheduled review date"] && (
                          <span className="text-xs text-gray-600 tabular-nums">
                            {formatDate(r["Scheduled review date"])}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {r["Permit expiry date"] && (
                        <span className="text-xs text-gray-600 tabular-nums">
                          {formatDate(r["Permit expiry date"])}
                        </span>
                      )}
                    </div>
                  </div>
          ...
          {awardsState === "idle" || awardsState === "loading" ? (
            ...
            <div className="divide-y divide-gray-100">
              {memberAwards.length === 0 ? (
                <div className="px-4">
                  <EmptySection label="award records" />
                </div>
              ) : (
                memberAwards.map((r, i) => (
                  <div
                    key={`${r.Accreditation}-${i}`}
                    className="flex items-center justify-between px-4 py-3 bg-purple-50 border-purple-200"
                  >
                    <div className="flex items-center gap-3">
