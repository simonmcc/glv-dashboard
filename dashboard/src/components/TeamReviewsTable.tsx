...
export function TeamReviewsTable({
  records,
  isLoading,
  searchTerm = "",
  memberNameMap,
}: TeamReviewsTableProps) {
  ...
  const filteredRecords = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      switch (sortField) {
        case "leader":
          comparison = resolveName(a, memberNameMap).localeCompare(
            resolveName(b, memberNameMap),
          );
          break;
        case "date": {
          const aDate = a["Scheduled review date"]
            ? new Date(a["Scheduled review date"]).getTime()
            : Infinity;
          const bDate = b["Scheduled review date"]
            ? new Date(b["Scheduled review date"]).getTime()
            : Infinity;
        ...
        case "overdue": {
          const aOverdue =
            a["Review overdue"]?.toLowerCase() === "yes" ||
            a["Review overdue"] === "true"
              ? 1
              : 0;
          const bOverdue =
            b["Review overdue"]?.toLowerCase() === "yes" ||
            b["Review overdue"] === "true"
              ? 1
              : 0;
          ...
          <tbody className="divide-y divide-gray-100">
            {filteredRecords.length === 0 ? (
              ...
              filteredRecords.map((record, index) => {
                const isOverdue =
                  record["Review overdue"]?.toLowerCase() === "yes" ||
                  record["Review overdue"] === "true";
                return (
                  <tr
