...
describe("JoiningJourneyProgress", () => {
  ...
  it("shows Delivering a Great Programme as not-started when absent from learningRecords", () => {
    ...
    expect(
      screen.getByText(/Delivering a Great Programme/),
    ).toBeInTheDocument();
  });
  ...
  it('shows fallback "Growing Roots" chip when all GR modules are done', () => {
    const allDoneLearning: LearningRecord[] = [
      ...
      },
      {
        ...
        "Last name": "Brown",
        "Membership number": "22222",
        Learning: "Leading Scout Volunteers",
        Status: "Valid",
        "Expiry date": null,
      },
      {
    ...
    render(
      <JoiningJourneyProgress
        {...defaultProps}
        learningRecords={allDoneLearning}
      />,
    );
  ...
  it("renders a loading skeleton when isLoading is true", () => {
    const { container } = render(
      <JoiningJourneyProgress {...defaultProps} isLoading={true} />,
    );
  ...
  it("shows empty state when no members are in the joining journey", () => {
    render(
      <JoiningJourneyProgress {...defaultProps} joiningJourneyRecords={[]} />,
    );
    expect(
      screen.getByText(/No members currently in their joining journey/),
    ).toBeInTheDocument();
  });
  ...
  it("shows member count in the toolbar", () => {
    render(<JoiningJourneyProgress {...defaultProps} />);
    expect(
      screen.getByText(/2 members in joining journey/),
    ).toBeInTheDocument();
  });
  ...
  it("renders chips for unknown outstanding items not in JOURNEY_STEPS or known categories", () => {
    const records: JoiningJourneyRecord[] = [
      ...
      },
      {
        ...
        "Last name": "Smith",
        "Membership number": "33333",
        Item: "dataProtectionTrainingComplete",
        Status: "Incomplete",
      },
      {
        "First name": "Eve",
    ...
    expect(
      screen.getByText(/dataProtectionTrainingComplete/),
    ).toBeInTheDocument();
